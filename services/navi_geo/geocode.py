"""
navi-geo geocode — structured preprocessing, multi-source retrieval, reranking.

Faithful port of recon's ``lib/geocode.py``. Behaviour-identical to recon; the
only changes are (a) the Photon base URL is read from ``PHOTON_URL`` instead of
a hardcoded constant, and (b) the reranking-audit trace log is opt-in via
``NAVI_GEO_RERANK_TRACE_LOG`` (recon always wrote ``/tmp/geocode_rerank_trace.log``
at import; here no FileHandler is attached unless the env var is set).
...
logger = logging.getLogger('navi_geo.geocode')

# ── Photon base URL (single source of truth for the default) ──
DEFAULT_PHOTON_URL = "http://localhost:2322"


def photon_url():
    """Photon base URL, env-overridable. Read per-call so tests/env apply."""
    return os.environ.get('PHOTON_URL', DEFAULT_PHOTON_URL)


# ── Trace logger for reranking audit (opt-in) ──
# recon attached a DEBUG FileHandler to /tmp at import unconditionally. Here the
# handler is attached only when NAVI_GEO_RERANK_TRACE_LOG names a path; otherwise
# the logger stays silent and cheap (level above DEBUG short-circuits formatting).
_trace_logger = logging.getLogger('navi_geo.geocode.trace')
...
def _setup_trace_logger():
    ...
    path = os.environ.get('NAVI_GEO_RERANK_TRACE_LOG')
    if path:
        ...
        # No sink: NullHandler + level above DEBUG so .debug() calls are no-ops.
