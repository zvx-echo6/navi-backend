"""Address Book — YAML-backed saved-location lookup (navi-geo's reader copy).

Behavior-identical port of recon's ``lib/address_book.py`` — the same copy
navi-contacts ships. navi-geo only *reads* this file (for the geocode nickname
short-circuit + the 75 m result annotation); navi-contacts owns the writes/UI.
Both read the shared YAML via ``NAVI_ADDRESS_BOOK_YAML`` (Phase-B address-book
decision: Option B / shared-file read — see the PR description).

Named locations (home, work, etc.) with fuzzy matching over name + aliases +
partial address. Hot-reloads when the YAML's mtime changes.
"""
import logging
import os
import re
...
logger = logging.getLogger('navi_geo.address_book')
