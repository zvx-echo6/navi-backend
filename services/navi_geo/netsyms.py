"""
navi-geo Netsyms AddressDatabase2025 — SQLite-backed US+CA address lookup.

Faithful port of recon's ``lib/netsyms.py``. Only change: the DB path is read
from ``NAVI_NETSYMS_DB`` (recon hardcoded it). The ~35 GB file stays external
and read-only.
...
logger = logging.getLogger('navi_geo.netsyms')

DEFAULT_DB_PATH = '/mnt/nav/addresses/AddressDatabase2025.sqlite'


def db_path():
    """Netsyms SQLite path, env-overridable via NAVI_NETSYMS_DB."""
    return os.environ.get('NAVI_NETSYMS_DB', DEFAULT_DB_PATH)
...
def _get_conn():
    ...
    with _lock:
        ...
        logger.info("Netsyms DB opened: %s", path)
        return _conn
