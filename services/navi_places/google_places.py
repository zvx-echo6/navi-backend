"""Google Places (New) API client — port of recon's lib/google_places.py.

Tertiary gap-fill enrichment (opening_hours/phone/website) for business POIs.
Daily-cap counter + per-place results cached in place_cache.db. Changes vs
recon: stdlib logging; DB access via the shared place_cache connection
(place_cache.py owns the schema, incl. google_api_calls); daily cap from env
GOOGLE_PLACES_DAILY_CAP (default DEFAULT_DAILY_CAP).
"""
import json
import logging
import os
...
logger = logging.getLogger('navi_places.google_places')
...
def _get_db():
    """Shared place_cache connection (place_cache.py owns the schema)."""
    return place_cache.get_conn()
...
def _get_daily_cap():
    """Daily API call cap — env GOOGLE_PLACES_DAILY_CAP or the default."""
    try:
        return int(os.environ.get('GOOGLE_PLACES_DAILY_CAP', DEFAULT_DAILY_CAP))
...
def search_place(name, lat, lon, radius_m=200):
    """Search Google Places (New) for a business by name + location. Returns place id or None."""
    key = _get_api_key()
    if not key:
        return None
...
def get_place_details(place_id):
    """Fetch details for a Google Place ID. Returns dict or None."""
    key = _get_api_key()
    if not key:
        return None
    if not check_daily_cap():
    ...
    try:
        ...
        logger.info("google_places: action=details id=%s result=hit", place_id)
        return result
...
def _periods_to_osm(periods):
    """Convert Google Places periods array to an OSM opening_hours string."""
    if not periods:
        return None
