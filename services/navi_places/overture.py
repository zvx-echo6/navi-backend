"""Overture Maps enrichment — verbatim port of recon's lib/overture.py.

Lazy module-level psycopg2 SimpleConnectionPool(1,3) to OVERTURE_DB_*, with the
_pool_failed graceful-degradation latch. Adds reset_pool() + probe_db() for
create_app() / admin-info (mirroring navi_landclass/db.py).
"""
import json
import logging
import os
...
logger = logging.getLogger('navi_places.overture')
...
def reset_pool():
    """Drop the cached pool/latch so the next call re-reads OVERTURE_DB_* env."""
    global _pool, _pool_failed
    if _pool is not None:
        try:
            _pool.closeall()
...
def probe_db():
    """Quick SELECT 1 health probe for admin-info. Returns (ok, detail)."""
    pool = _get_pool()
    if pool is None:
...
def find_by_osm_id(osm_type, osm_id):
    """Look up an Overture place by its OSM cross-reference. Returns dict or None."""
